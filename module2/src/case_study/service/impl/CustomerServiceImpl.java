            ...
            hoTen=scanner.nextLine();
             try {
                 if(!hoTen.matches("^([A-Z][a-z]{1,})\\s([A-Z][a-z]{0,})+(\\s([A-Z][a-z]{0,}+))?$")){
                     throw new CustomerCodeFormatException("Lỗi cú pháp(vd:Nguyen Van AN, Nguyen An)");
                 }
                 break;
        ...
        while (true){
            ...
            now=now.plusYears(-18);
                if (localDate.isAfter(now)){
                    throw new NotYet18YearOldException("Chưa đủ 18");
                }
                now=now.plusYears(-82);
                if (localDate.isBefore(now)){
                    throw new Over100YearsOldException("Hơn 100");
                }
                break;
