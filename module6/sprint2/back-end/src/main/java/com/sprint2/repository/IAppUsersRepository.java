...
public interface IAppUsersRepository extends JpaRepository<AppUsers, Integer> {
    @Query(value = "select * from app_users a where a.username = :name and status = 0", nativeQuery = true)
    AppUsers findAppUserByName(@Param("name") String name);

    @Query(value = "SELECT username from app_users where username = ?1 and status = 0", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "update app_users set status = 1 where id = :id", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "insert into app_users (username, `password`, creation_date, status) values (:username, :password, current_date(), 0)", nativeQuery = true)
